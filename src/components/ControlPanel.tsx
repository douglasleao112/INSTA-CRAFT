...
import { AspectRatio, Branding, SlideData, SignatureSlot } from "../types";
...
export const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  const [uploadedImages, setUploadedImages] = useState<string[]>(() => {
    const salvo = localStorage.getItem("fotos_carrossel");
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    const salvo = localStorage.getItem("chat_carrossel");
    return salvo
      ? JSON.parse(salvo)
      : [
          {
            role: "model",
            text:
              "Bem-vindo ao Engine Content PRO \n\n" +
  ...
  // Persistir config
  useEffect(() => {
    localStorage.setItem("config_carrossel", JSON.stringify(config));
  }, [config]);

  // Persistir chat
  useEffect(() => {
    localStorage.setItem("chat_carrossel", JSON.stringify(messages));
  }, [messages]);

  // Persistir fotos
  useEffect(() => {
    if (uploadedImages.length > 0) {
      localStorage.setItem("fotos_carrossel", JSON.stringify(uploadedImages));
    }
  }, [uploadedImages]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const chat = ai.chats.create({
        ...
        config: { systemInstruction: ENGINE_PROMPT },
      ...
      setMessages((prev) => [
        ...prev,
        { role: "model", text: "Erro ao conectar com a inteligência artificial. Tente novamente." },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleBrandingChange = (key: keyof Branding, value: any) => {
    updateConfig({ branding: { ...config.branding, [key]: value } });
  };
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer group">
                      <div
                        onClick={() =>
                          updateConfig({ isGlobalBranding: !config.isGlobalBranding })
                        }
                        className={cn(
                  ...
                  <div className="grid grid-cols-5 gap-2 bg-gray-50 p-3 rounded-2xl border border-black/5">
                    {(Object.entries(config.branding.signatures) as [
                      keyof Branding["signatures"],
                      SignatureSlot
                    ][]).map(([key, slot]) => {
                    ...
                    {(Object.entries(config.branding.signatures) as [
                      ...
                      .map(([key, slot]) => {
                        ...
                        return (
                          <div
                            key={key}
                            className="bg-gray-50/50 p-4 rounded-2xl border border-black/5 space-y-4"
                          >
                            <div className="flex items-center justify-between">
                                    ...
                                    return (
                                      ...
                                      >
                                        <div
                                          onClick={() =>
                                            updateSlot({
                                              [opt.id]: !slot[opt.id as keyof SignatureSlot],
                                            } as any)
                                          }
                                          className={cn(
                                            "w-4 h-4 rounded border flex items-center justify-center transition-all",
                  ...
                  {/* Foto da assinatura (aparece se alguma assinatura ativa tiver FOTO marcada) */}
                  {shouldShowAvatarSection && (
                    <div className="pt-6 border-t border-black/5 space-y-4">
                      ...
                      <div className="bg-gray-50 p-5 rounded-3xl border border-black/5 flex items-center gap-6">
                        ...
                        >
                          {Object.values(config.branding.signatures).find(
                            (s: any) => s.enabled && s.avatar
                          ) ? (
                            <>
                              <img
                                src={
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  handleBrandingChange(
                                    "signatures",
                                    Object.fromEntries(
                                      Object.entries(config.branding.signatures).map(([k, v]) => [
                                        k,
                                        { ...(v as any), avatarBorderWidth: width },
                                      ])
                                    )
                                  );
                                }}
                              ...
                              <div className="relative w-8 h-8 rounded-lg border border-black/10 overflow-hidden shadow-sm">
                                <input
                                  ...
                                  value={
                                    (Object.values(config.branding.signatures).find(
                                      (s: any) => s.enabled
                                    ) as any)?.avatarBorderColor || "#ffffff"
                                  }
                                  onChange={(e) => {
                                    ...
                                    handleBrandingChange(
                                      "signatures",
                                      Object.fromEntries(
                                        Object.entries(config.branding.signatures).map(
                                          ([k, v]) => [
                                            k,
                                            { ...(v as any), avatarBorderColor: color },
                                          ]
                                        )
                                      )
                                    );
                              ...
                              onChange={(e) => {
                                ...
                                handleBrandingChange(
                                  ...
                                  Object.fromEntries(
                                    Object.entries(config.branding.signatures).map(([k, v]) => [
                                      k,
                                      { ...(v as any), avatarBorderRadius: radius },
                                    ])
                                  )
                                );
                              }}
                  ...
                  {/* Moldura (aparece se alguma assinatura ativa tiver MOLDURA marcada) */}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handleBrandingChange("container", {
                                  ...config.branding.container,
                        ...
                        <div className="space-y-3">
                          <div className="space-y-1">
                            <div className="flex justify-between text-[9px] font-bold uppercase text-gray-400">
                              ...
                              <span className="text-indigo-600">
                                {config.branding.container.opacity}%
                              </span>
                            </div>
                            <input
                              type="range"
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between text-[9px] font-bold uppercase text-gray-400">
                              ...
                              <span className="text-indigo-600">
                                {config.branding.container.borderRadius}px
                              </span>
                            </div>
                            <input
                              type="range"
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {uploadedImages.slice(0, 8).map((img, i) => (
                          <div
                            key={i}
                            className="aspect-square rounded-lg overflow-hidden border border-black/5 bg-gray-100"
                          >
